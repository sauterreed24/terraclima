...
- **Livability lens v2.1:** The hero "Livability blend" score blends felt comfort, a tail-risk-aware hazard cushion (55% mean-of-9 + 45% max-of-9), U-shaped precipitation moderation, curated resilience and growability, and a new lived-friction axis (cost pressure / social-fabric stress / daily-services access). Climate-perfect fog-belt coasts and ferry-only Pacific outposts that used to monopolise Live-fit, retirement, and remote-work rankings now sit alongside cost, social, and access realities resident-review research and cost-of-living indices flag. Sky comfort applies an explicit summer-sunshine-collapse penalty for measured-dim summers. Per-component breakdowns and a "Lived signals" section appear inside every place profile with cited sources so you can see why a place ranked where it did. See [`src/lib/livability-score.ts`](src/lib/livability-score.ts) and [`src/lib/live-fit.ts`](src/lib/live-fit.ts).
