...
- **Shortlist synthesis:** `src/lib/explorer-scout-brief.ts` and `src/lib/decision-matrix.ts` turn the active Explorer ranking into a deterministic field brief: best current match, leader rationale, easy-month runway, climate/risk spread, dominant microclimate family, decision rows, and compare-ready leader IDs.
