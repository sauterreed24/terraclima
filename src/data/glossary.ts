...
export const CONCEPTS: Concept[] = [
  ...
  {
    id: "de-martonne-aridity",
    term: "De Martonne aridity index",
    short: "MAP ÷ (MAT + 10) — annual precipitation per degree of mean warmth, with bands from hyperarid to extremely humid.",
    long: "Emmanuel de Martonne's 1926 index normalizes annual precipitation against mean annual temperature plus 10 °C, so that warmer regions need more rain to reach the same band. Le Houérou's refinement places the bands as: hyperarid (<5), arid (5–10), semi-arid (10–20), Mediterranean (20–24), sub-humid (24–28), humid (28–35), very humid (35–55), extremely humid (≥55). The index becomes undefined as MAT approaches −10 °C, which Terraclima handles by reporting no value for sub-Arctic places.",
    mechanism: "A water-vs-warmth ratio with a temperature-floor shift; sensitive to both MAP and MAT but blind to seasonality.",
    exampleIds: ["sequim-wa", "yuma-az", "hilo-hi"],
  },
  {
    id: "conrad-continentality",
    term: "Conrad continentality index",
    short: "K = 1.7·A / sin(φ + 10°) − 14 — annual temperature swing corrected for latitude, where 0 is pure oceanic and ~100 is Verkhoyansk.",
    long: "Victor Conrad's 1946 formulation isolates how strongly a place's annual temperature swing is driven by continentality versus latitude. The +10° latitude offset prevents the index from blowing up near the equator; very low or negative K values are legitimate signals of tropical hyper-oceanic regimes with tiny annual amplitude. Terraclima bands run extreme oceanic / oceanic / sub-continental / continental / extreme continental.",
    mechanism: "Annual range A = (warmest mean − coldest mean); latitude correction acknowledges that high-latitude oceanic stations still see large swings.",
    exampleIds: ["hilo-hi", "monterey-ca", "fairbanks-ak"],
  },
  {
    id: "thornthwaite-pet",
    term: "Thornthwaite PET",
    short: "Monthly potential evapotranspiration estimated from temperature + daylength alone — atmospheric water demand without needing radiation or humidity.",
    long: "C. W. Thornthwaite's 1948 formulation derives PET from monthly mean temperature, an annual heat index, and a daylength correction based on latitude. It's the most widely used PET method when only temperature data is available, although it is known to overshoot in very hot climates (Phoenix-style desert summers). Terraclima reports annual PET in mm/yr so it can be compared directly against annual precipitation.",
    mechanism: "Heat index I = Σ(T/5)^1.514 over months with T > 0; monthly PET scales as 16·(10·T/I)^a corrected by (N/12)·(days/30).",
    exampleIds: ["sequim-wa", "yuma-az", "hilo-hi"],
  },
  {
    id: "selianinov-htc",
    term: "Selianinov hydrothermal coefficient (HTC)",
    short: "Growing-season precipitation against the cumulative warmth of months above 10 °C — the canonical Russian agroclimatology measure.",
    long: "G. T. Selianinov's 1928 coefficient compares growing-season rainfall against the sum of daily mean temperatures across months whose mean exceeds 10 °C (the 'active growing season'). HTC < 0.4 is classically dry, 1.0–1.3 is sufficient moisture, and >1.6 is wet. Terraclima uses the standard monthly substitution: ΣT_daily ≈ Σ(T_mean · days_in_month), without which the index would mislabel rain-shadow towns as wet.",
    mechanism: "Decouples agricultural water balance from total-annual stats — Sequim has 1700 mm of winter rain but is dry by HTC during its growing season.",
    exampleIds: ["sequim-wa", "hilo-hi", "osoyoos-bc"],
  },
  {
    id: "unep-aridity",
    term: "UNEP aridity index (P/PET)",
    short: "Annual precipitation divided by Thornthwaite PET — the UN's working definition of desertification risk.",
    long: "The 1992 UNEP World Atlas of Desertification codified this ratio as the standard land-surface aridity measure: hyperarid (<0.05), arid (0.05–0.20), semi-arid (0.20–0.50), dry sub-humid (0.50–0.65), humid (≥0.65). It combines De Martonne's water-vs-warmth instinct with Thornthwaite's PET model so the denominator is a physically grounded estimate of evaporative demand rather than just temperature.",
    mechanism: "A balance-style index: AI > 1 means precipitation exceeds atmospheric water demand on an annual basis (moisture surplus); AI ≪ 1 means soils dry by default.",
    exampleIds: ["sequim-wa", "yuma-az", "hilo-hi"],
  },
];
