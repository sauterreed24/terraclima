...
- **Quieter chrome (`src/components/AtlasMap.tsx`, `src/styles.css`):** the Atlas readout collapses to Leaders + Driver until hover/focus-within (coarse pointers stay collapsed). Rank trails default to quiet until `k ≥ 1.35` outside clusters; country labels fade earlier. Cluster picker max height caps at 380 px.
