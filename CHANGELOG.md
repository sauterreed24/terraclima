...
### Visual quality pass (dark-theme legibility + vs-home polish)

- **Dark-theme tonal text remap (`src/styles.css`):** the `--color-{glacier,sage,ochre,ember}-700` emphasis tokens were tuned for light paper (`#0d5f72`, `#245c36`, `#a86a0c`, `#a33418`) and were never remapped for the moonlit theme, leaving sub-2:1 text across the app in dark mode — dossier eyebrows ("The delta read", comfort-read headlines), the OpenStreetMap dossier link, every card's sage **Annual precip** stat, bioclim edge notes, and tourism icons. A usage audit confirmed all `-700` references are text-only (zero background/border usages in CSS or markup), so `:root[data-theme="dark"]` now remaps the tier to bright equivalents (≥ 7:1 on panel surfaces). Verified by headless-browser screenshots and computed-style probes in both themes.
- **Dossier signature panel readable in dark (`src/styles.css`):** the header's "Place signature / Feel / Leads with / Verify" panel kept its light-paper cell backgrounds under dark-remapped light text, washing out the primary label entirely. It now gets the same moonlit treatment as the card signature band. The light rule's `!important` was replaced with a child-combinator selector of equal effect so the unlayered dark override can win the cascade — light theme is pixel-identical.
- **Hero credit line readable in dark (`src/components/PlaceDetail.tsx`, `src/styles.css`):** the Wikimedia credit strip under dossier hero images kept its cream background while its text remapped to moonlit light — light-on-light. Extracted to a `.tc-hero-credit` class with a dark surface override.
- **Vs-home section polish (`src/components/place-detail/PlaceVersusHome.tsx`, `src/styles.css`):** the headline sentence now sits in the house `tc-accent-panel` treatment (matching the Livability comfort read); delta tiles are flex columns with the descriptor chip pinned to the bottom edge so rows stay aligned when labels wrap; descriptor chips render sentence-case instead of uppercase; the "higher" arrow uses `ochre-700` so it carries contrast in both themes.
- **Vs-home chip case consistency (`src/components/CompareView.tsx`, `src/styles.css`):** Compare-column delta chips were uppercase while card chips were sentence-case; a shared `.tc-home-delta-chips` rule normalises both to sentence case (they read as data values, not labels).
- **Print from dark theme produces a readable brief (`src/styles.css`):** the moonlit variable remap (`:root[data-theme="dark"]`) used to keep applying under `@media print`, so a dossier printed from dark mode rendered near-white ink (`--color-ice: #f2f5fb`) on the forced white page. The whole dark token block is now scoped inside `@media not print`, so paper output always reads as the day theme regardless of the active screen theme. Verified with print-media emulation: on screen `--color-glacier-700` resolves to the bright remap, under print it reverts to day ink with a white body.
